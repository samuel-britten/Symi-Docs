...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let polynomial = session.parse("x^2 + 2*x + 1")?;
    let exact_rational = session.rational(2, 3)?;

    println!("{}", polynomial.factor()?);
    println!("{}", polynomial.differentiate(&x)?);
    println!("{}", exact_rational);
