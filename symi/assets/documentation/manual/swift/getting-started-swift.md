...
let session = UniffiSession()
let x = try session.symbol(name: "x")
let polynomial = try session.parse(text: "x^2 + 2*x + 1")

print(try polynomial.factor().plainText())
print(try session.integrate(target: try session.sin(argument: x), variable: "x").plainText())
```
